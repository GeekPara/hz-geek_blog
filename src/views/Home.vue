<template>
  <div class="mdui-col-xs-12 mdui-col-md-10 mdui-col-offset-md-1" id="articleListHolder">
    <div class="mdui-card mdui-hoverable mdui-m-y-2" v-for="article in articles" :key="article.title">
...
export default {
  ...
  created () {
    const AV = this.AV
    loadMainArticleList();
    async function loadMainArticleList() {
      ...
      for (let article of articles) {
        ...
        layout.push({title:title,subtitle:subtitle,headImage:headImage,avatar:avatar,authorName:authorName,date:date})
      }
    }
  }
}
